...
data.representation <- c("indicators", "squared") # c("ids", "indicators", "setindicators", "clean")
...
for (iter in iter.range) {
    ...
    result$hazard$true <- sqrt(result$hazard$true)
    result$hazard$predicted <- sqrt(result$hazard$predicted)
    result$submission[,"Hazard"] <- sqrt(result$submission[,"Hazard"])
    result$solution[,"Hazard"] <- sqrt(result$solution[,"Hazard"])
