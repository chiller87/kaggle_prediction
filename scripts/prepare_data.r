...
prepare_data <- function(data, column.name.range) {
    
    # remove columns with negative feature importance
    negative.feature.importance <- c("T1_V10", "T1_V13", "T2_V7", "T2_V10")
    data <- data[,!(names(data) %in% negative.feature.importance)]
...
replace_with_id <- function(data, column.name.range) {
    columns.to.drop <- c()
