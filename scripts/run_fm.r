...
data <- prepare_data(data = raw_data, column.name.range = non.numeric.columns)
write.table(data, file = "tmp/data_with_indicators.csv", sep = ",", row.names = FALSE, col.names = TRUE, append = FALSE)
#print("done!")

print("preparing test data ...")
test.data <- NA
test.raw_data <- read.table("../data/test.csv", sep = ",", header = TRUE)
test.data <- prepare_data(data = test.raw_data, column.name.range = non.numeric.columns)
...
stdev.range <- c(0)# seq(0.02, 0.1, 0.02)#c(0.0) 
iter.range <- c(400) #seq(400, 1600, 200) 
...
for (iter in iter.range) {
    ...
    result <- fm_prediction(data=data, data.test=test.data, some.params=list(method=fm.method, iter=iter, k=k, stdev=stdev, reg=reg, lr=lr))
